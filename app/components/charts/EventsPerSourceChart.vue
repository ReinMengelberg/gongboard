...
// Accept a simple counts map per source, aggregated by EventTransformer.transformPerSource
...
// Only one category: total events
  ...
  <Card class="p-4 h-full">
    <CardHeader class="pt-2">
      <div class="flex min-w-0 flex-col">
        <CardTitle>{{ title || 'Events per Source' }}</CardTitle>
    ...
    <CardContent class="pt-0">
      <div :style="{ height: `${height ?? 420}px` }">
        ...
        />
        <div v-else class="text-sm text-muted-foreground">No per-source event data for selected range.</div>
      </div>
    </CardContent>
  </Card>
</template>
